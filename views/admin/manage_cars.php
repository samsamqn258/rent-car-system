                        ...
                        <table class="table table-striped table-hover" id="carsTable">
                            ...
                            <tbody>
                                <?php foreach ($cars as $car): ?>
                                    <tr data-status="<?php echo $car['status']; ?>" data-type="<?php echo $car['car_type']; ?>" data-search="<?php echo strtolower($car['brand'] . ' ' . $car['model'] . ' ' . $car['owner_name']); ?>">
                                        <td><?php echo $car['id']; ?></td>
                                        <td>
                                            <?php if (!empty($car['primary_image'])): ?>
                                                <img src="<?php echo BASE_URL . '/' . $car['primary_image']; ?>" alt="<?php echo $car['brand'] . ' ' . $car['model']; ?>" class="img-thumbnail" style="width: 60px; height: 40px; object-fit: cover;">
                                            <?php else: ?>
                                                <img src="<?php echo BASE_URL; ?>/public/images/no-image.jpg" alt="No Image" class="img-thumbnail" style="width: 60px; height: 40px; object-fit: cover;">
                                            <?php endif; ?>
                                        </td>
                                        <td>
                                        ...
                                        <td>
                                            <div class="btn-group">
                                                <a href="<?php echo BASE_URL; ?>/cars/details/<?php echo $car['id']; ?>" class="btn btn-sm btn-info" target="_blank">
                                                    <i class="fas fa-eye"></i>
                                                </a>
                                                
                                                <?php if ($car['status'] == 'unapproved'): ?>
                                                    <a href="<?php echo BASE_URL; ?>/cars/approve/<?php echo $car['id']; ?>" class="btn btn-sm btn-success">
                                                        <i class="fas fa-check"></i>
                                                    </a>
                                                    <button type="button" class="btn btn-sm btn-danger" data-bs-toggle="modal" data-bs-target="#rejectModal<?php echo $car['id']; ?>">
                                                        <i class="fas fa-times"></i>
                                                    </button>
                                                    
                                                    <!-- Reject Modal -->
                                                    <div class="modal fade" id="rejectModal<?php echo $car['id']; ?>" tabindex="-1" aria-labelledby="rejectModalLabel<?php echo $car['id']; ?>" aria-hidden="true">
                                                ...
                                                <?php elseif ($car['status'] == 'approved' || $car['status'] == 'rented'): ?>
                                                    <button type="button" class="btn btn-sm btn-secondary" data-bs-toggle="modal" data-bs-target="#hideModal<?php echo $car['id']; ?>">
                                                        <i class="fas fa-eye-slash"></i>
                                                    </button>
                                                    
                                                    <!-- Hide Modal -->
                                                    <div class="modal fade" id="hideModal<?php echo $car['id']; ?>" tabindex="-1" aria-labelledby="hideModalLabel<?php echo $car['id']; ?>" aria-hidden="true">
                                                        <div class="modal-dialog">
                                                            <div class="modal-content">
                                                                <div class="modal-header">
                                                                ...
                                                                <div class="modal-body">
                                                                    <p>Bạn có chắc chắn muốn ẩn xe "<?php echo $car['brand'] . ' ' . $car['model']; ?>" không?</p>
                                                                    <p class="text-muted">Xe sẽ không hiển thị trong kết quả tìm kiếm nhưng vẫn tồn tại trong hệ thống.</p>
                                                                </div>
                                                                <div class="modal-footer">
                                                                    <button type="button" class="btn btn-secondary" data-bs-dismiss="modal">Đóng</button>
                                                                    <a href="<?php echo BASE_URL; ?>/cars/hide/<?php echo $car['id']; ?>" class="btn btn-primary">Ẩn xe</a>
                                                                </div>
                                                            </div>
                                                        </div>
                                                    </div>
                                                <?php elseif ($car['status'] == 'hidden'): ?>
                                                    <a href="<?php echo BASE_URL; ?>/cars/unhide/<?php echo $car['id']; ?>" class="btn btn-sm btn-primary">
